      ...
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbar">
          ...
          <ul class="navbar-nav ms-auto mb-2 mb-lg-0"> 
            ...
            <li class="nav-item pe-3"><a href="index.php" class="nav-link active " aria-current="page"> Acceuil</a>
            </li>
...
?>
    ...
    require_once('logout_u.php');
                if(isset($_SESSION['user_a'])){
                  echo '
                  <form method="POST">
                  <button name="déconnexion" class="btn btn-success">Déconnexion</button>
              </form>';
                }else{
                  echo '              <a href="abonner.php" class="nav-link" >
                  <input class="btn btn-success" type="submit" name="connect" value="connection/inscription"></a>
                  ';
                }
                ?>
    ...
    <section class="banner d-flex  align-items-center pt-">
      ...
      <h1 class="title-niv-1 text-white">tout les corrigés du site :</h1>
...
elseif (isset($_POST['Premiere_C'])) {
  ...
  extract($_POST);
  $req= $db->prepare('SELECT * FROM corriges WHERE  classe="Première D"  ORDER BY etablissement ');
  $req->execute();
  while($reponse = $req->fetch(PDO::FETCH_OBJ)){
      ?> 
       <div id="download" class="bg-white my-2">
  ...
  extract($_POST);
  $req= $db->prepare('SELECT * FROM corriges WHERE classe="Première TI"  ORDER BY etablissement ');
  $req->execute();
  while($reponse = $req->fetch(PDO::FETCH_OBJ)){
      ?> 
       <div id="download" class="bg-white my-2">
  ...
  extract($_POST);
  $req= $db->prepare('SELECT * FROM corriges WHERE  classe="Première A"  ORDER BY etablissement ');
  $req->execute();
  while($reponse = $req->fetch(PDO::FETCH_OBJ)){
      ?> 
       <div id="download" class="bg-white my-2">
  ...
  extract($_POST);
  $req= $db->prepare('SELECT * FROM corriges WHERE  classe="Terminale C"  ORDER BY etablissement ');
  $req->execute();
  while($reponse = $req->fetch(PDO::FETCH_OBJ)){
      ?> 
       <div id="download" class="bg-white my-2">
  ...
  extract($_POST);
  $req= $db->prepare('SELECT * FROM corriges WHERE  classe="Terminale D"  ORDER BY etablissement ');
  $req->execute();
  while($reponse = $req->fetch(PDO::FETCH_OBJ)){
      ?> 
       <div id="download" class="bg-white my-2">
  ...
  extract($_POST);
  $req= $db->prepare('SELECT * FROM corriges WHERE  classe="Terminale TI"  ORDER BY etablissement ');
  $req->execute();
  while($reponse = $req->fetch(PDO::FETCH_OBJ)){
      ?> 
       <div id="download" class="bg-white my-2">
  ...
  extract($_POST);
  $req= $db->prepare('SELECT * FROM corriges WHERE  classe="Terminale A"  ORDER BY etablissement ');
  $req->execute();
  while($reponse = $req->fetch(PDO::FETCH_OBJ)){
      ?> 
       <div id="download" class="bg-white my-2">
...
<?php } 
          ...
          $req= $db->prepare('SELECT * FROM corriges  ORDER BY etablissement ');
        $req->execute();
        while($reponse = $req->fetch(PDO::FETCH_OBJ)){
            ?> 
             <div id="download" class="bg-white my-2">
